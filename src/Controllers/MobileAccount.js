...
exports.updateMobileAccount = async (req, res) => {
  try {
      ...
      const updatedAccount = await MobileAccount.findByIdAndUpdate(
          req.params.id,
          { selectCompany: selectCompany?.trim(), mobileNumber: mobileNumber?.trim() },
...
exports.deleteMobileAccount = async (req, res) => {
  ...
  try {
      const deletedAccount = await MobileAccount.findByIdAndDelete({_id: req.params.id});
...
exports.getTodayLog = async (req, res) => {
  try {
      ...
      const debits = await Debit.find({ createdAt: { $gte: today, $lte: endOfDay } }).lean();
      const credits = await Credit.find({ createdAt: { $gte: today, $lte: endOfDay } }).lean();
