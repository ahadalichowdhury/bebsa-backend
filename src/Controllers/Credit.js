...
exports.createCustomer = async (req, res) => {
  try {
      ...
      const allowedCompanies = [
          'Bkash Personal', 'Bkash Agent', 'Nagad Personal', 'Nagad Agent',
          'Rocket Personal', 'Rocket Agent', 'Others'
      ];
      ...
      const mobileAccount = await MobileAccount.findOne({ mobileNumber: selectedAccount });
...
exports.updateCustomer = async (req, res) => {
    try {
        ...
        // Validate required fields
        if (!customerName || !customerNumber || !company || !selectedNumber || !entryBy || !selectedAccount) {
            return res.status(400).json({
                success: false,
                message: 'Missing required fields',
                requiredFields: ['customerName', 'customerNumber', 'company', 'selectedNumber', 'entryBy', 'selectedAccount']
            });
        }
        ...
        const existingCustomer = await Customer.findOne({ customerNumber: customerNumber.trim(), _id: { $ne: id } });
        if (existingCustomer) {
            return res.status(409).json({
                success: false,
                message: 'Customer number already exists',
        ...
        customer.newAmount = typeof newAmount === 'number' ? newAmount : customer.newAmount;
